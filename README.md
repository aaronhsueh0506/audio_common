...
| simd_kernel_nn | `include/simd_kernel_nn.h` | header-only **bit-exact** NEON/scalar kernels for the neural-network pre/post-processing (DFN2, Align-ULCNet, GTCRN, DeepVQE) and the resampler: branch-free all-finite scan, fill, elementwise multiply, feature power, Complex (de)interleave with optional scale, floored WOLA divide, and a multi-row polyphase ring dot. Kept apart from `simd_kernels.h` so the traditional AEC/NR kernel set never changes with NN work; it includes `simd_kernels.h` only for the NEON switch and the Complex-quad helpers. Same contract (scalar twins, `SIMD=0`, `-ffp-contract=off`); `make selftest` also runs `test/simd_nn_selftest.c`. |
